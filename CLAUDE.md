...
- **Every piece of custom logic (not Document Types, not Composers-as-registration, not Razor views — see the scope boundary in the TDD bullet above, it applies here too) gets a card in `docs/build/04_ARCHITECTURE_AND_PATTERNS_GUIDE.md`**: which OOP pillars, which SOLID principle(s), which design pattern if any (and "none beyond X" when that's the honest answer), and why. Decided 2026-09-02, alongside the TDD decision above — same reasoning: this is how a new dev and a veteran dev arrive at the same understanding of intent, and how a developer with no AI agent and no internet can extend this codebase consistently rather than guessing. Update it in the same phase that adds the logic, the same discipline `DEVLOG.md` already has. This documents what's actually there — it is not license to introduce structure §4a already rejected (DDD/Clean/Onion/Hexagonal, a Domain/Application/Infrastructure split, describing DI as ports-and-adapters). If you're naming a pattern to justify adding an abstraction rather than to explain one that already exists, stop.
